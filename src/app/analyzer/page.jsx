...
export default function App() {
  ...
  return (
    <div className="bg-[#020408] text-white min-h-screen relative overflow-x-clip flex flex-col items-center font-sans selection:bg-[#009DFF]/30 selection:text-[#009DFF]">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 bg-gradient-to-tr from-[#009DFF]/15 via-[#020408] to-[#020408]" />
      ...
      {bootComplete && isLoading && (
        <div className="relative z-10 w-full max-w-lg px-6 py-16 flex flex-col items-center justify-center animate-in fade-in duration-700 my-auto">
          ...
          {/* Progress bar */}
          <div className="w-full max-w-sm h-1.5 bg-white/10 backdrop-blur-md rounded-full overflow-hidden mb-6 border border-white/20 dark:border-white/5">
      ...
      {bootComplete && !isLoading && !analysisResult && (
        <main className="relative z-10 w-full max-w-2xl px-6 py-12 animate-in fade-in slide-in-from-bottom-8 duration-700 my-auto">
          <div className="text-center mb-12 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#009DFF]/10 text-[#009DFF] text-sm font-semibold mb-6 border border-[#009DFF]/30 backdrop-blur-md shadow-sm">
              <span className="w-2 h-2 rounded-full bg-[#009DFF] animate-pulse shadow-[0_0_8px_#009DFF]" />
              AI-Powered Career Intelligence
            </div>
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-6 bg-clip-text text-transparent bg-gradient-to-r from-[#009DFF] to-[#8c52ff] drop-shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-8 bg-[#0a0f1a]/40 backdrop-blur-3xl border border-[#009DFF]/20 rounded-[2.5rem] p-8 md:p-10 shadow-[0_8px_32px_rgba(0,157,255,0.08)] relative overflow-hidden group transition-all duration-500 hover:shadow-[0_8px_40px_rgba(0,157,255,0.2)]">
            ...
            <div className="space-y-3 relative z-10">
              <label className="flex items-center gap-2 text-sm font-bold text-white/80 uppercase tracking-widest">
                <span className="w-6 h-6 rounded-full bg-[#009DFF]/20 text-[#009DFF] flex items-center justify-center text-xs backdrop-blur-sm border border-[#009DFF]/30">1</span>
                Upload Resume
              </label>
              
              <div className="w-full h-32 border border-[#009DFF]/20 rounded-2xl flex flex-col items-center justify-center gap-2 bg-[#0a0f1a]/40 backdrop-blur-md hover:bg-[#009DFF]/10 transition-all cursor-pointer group/drop shadow-inner">
                {!resumeFile ? (
                  <>
                    <Upload className="w-6 h-6 text-[#009DFF]/60 group-hover/drop:text-[#009DFF] transition-colors" />
                    <span className="text-sm text-white/60">Click to upload PDF or DOCX</span>
                  </>
                ) : (
            ...
            <div className="space-y-3 relative z-10">
              <label className="flex items-center gap-2 text-sm font-bold text-white/80 uppercase tracking-widest">
                <span className="w-6 h-6 rounded-full bg-[#009DFF]/20 text-[#009DFF] flex items-center justify-center text-xs backdrop-blur-sm border border-[#009DFF]/30">2</span>
                Targeted Job Role
              </label>
              <input 
                required 
                ...
                className="w-full bg-[#020408]/60 backdrop-blur-md border border-[#009DFF]/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#009DFF] focus:ring-1 focus:ring-[#009DFF] transition-all shadow-inner" 
            ...
            <div className="space-y-3 relative z-10">
              <label className="flex items-center gap-2 text-sm font-bold text-white/80 uppercase tracking-widest">
                <span className="w-6 h-6 rounded-full bg-[#009DFF]/20 text-[#009DFF] flex items-center justify-center text-xs backdrop-blur-sm border border-[#009DFF]/30">3</span>
                Job Description <span className="text-xs text-white/40 normal-case">(Optional)</span>
              </label>
              
              <div className="flex bg-[#0a0f1a]/40 backdrop-blur-md rounded-xl p-1 w-full max-w-[240px] mb-2 border border-[#009DFF]/20 shadow-inner">
                <button type="button" onClick={() => setJdMode('type')} className={`flex-1 py-1.5 text-xs font-medium rounded-lg transition-all ${jdMode === 'type' ? 'bg-[#009DFF]/20 text-white shadow-sm border border-[#009DFF]/30' : 'text-white/40 hover:text-white/80'}`}>Type / Paste</button>
                <button type="button" onClick={() => setJdMode('upload')} className={`flex-1 py-1.5 text-xs font-medium rounded-lg transition-all ${jdMode === 'upload' ? 'bg-[#009DFF]/20 text-white shadow-sm border border-[#009DFF]/30' : 'text-white/40 hover:text-white/80'}`}>Upload File</button>
              </div>
              ...
              {jdMode === 'type' ? (
                <textarea 
                  ...
                  className="w-full h-32 bg-[#020408]/60 backdrop-blur-md border border-[#009DFF]/20 rounded-xl p-4 text-white focus:outline-none focus:border-[#009DFF] transition-all resize-none shadow-inner" 
                  placeholder="Paste the job description here..." 
                />
              ) : (
                <div className="w-full h-32 border border-[#009DFF]/20 rounded-xl flex flex-col items-center justify-center gap-2 bg-[#0a0f1a]/40 backdrop-blur-md hover:bg-[#009DFF]/10 transition-all cursor-pointer group/drop shadow-inner">
                  {!jdFile ? (
                    <>
                      <Upload className="w-6 h-6 text-[#009DFF]/60 group-hover/drop:text-[#009DFF] transition-colors" />
                      <span className="text-xs text-white/60 text-center px-4">Upload PDF, DOCX, or Image (JPG/PNG)</span>
                    </>
                  ) : (
            ...
            <div className="space-y-3 relative z-10">
              <label className="flex items-center gap-2 text-sm font-bold text-white/80 uppercase tracking-widest">
                <span className="w-6 h-6 rounded-full bg-[#009DFF]/20 text-[#009DFF] flex items-center justify-center text-xs backdrop-blur-sm border border-[#009DFF]/30">4</span>
                Current Challenges <span className="text-xs text-white/40 normal-case">(Optional)</span>
              </label>
              <textarea 
                value={userProblems} 
                onChange={(e) => setUserProblems(e.target.value)} 
                className="w-full h-24 bg-[#020408]/60 backdrop-blur-md border border-[#009DFF]/20 rounded-xl p-4 text-white focus:outline-none focus:border-[#009DFF] transition-all resize-none shadow-inner" 
                placeholder="e.g. I keep failing technical rounds, or I am not getting callbacks..." 
              />
            </div>

            <button type="submit" disabled={isLoading} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#009DFF] to-[#8c52ff] text-white px-6 py-4 rounded-full font-bold text-[15px] hover:scale-[1.02] active:scale-95 transition-all shadow-[0_0_20px_rgba(0,157,255,0.3)] hover:shadow-[0_0_30px_rgba(0,157,255,0.5)] relative z-10 mt-4 disabled:opacity-70 disabled:pointer-events-none">
              {isLoading ? (
